...
function SearchPost({post, id}) {
  ...
  useEffect(() => {
    if(!userpost || !userpost.constituency || !id) return;
...
useEffect(() => {
 ...
 const submitReport = async () => {
  ...
  return (
    ...
    <div className={`w-full ${isHidden ? 'hidden' : 'my-2 border-[1px] border-gray-300 dark:border-gray-900 rounded-md mt-1 p-2'}`}>
      ...
      {post?.data()?.citeInput ? (<div><p onClick={() => router.push(`/constituencyposts(id)/${id}`)}></p>{post?.data()?.citeInput}
        <div className="border rounded-md border-gray-300 dark:border-gray-900 hover:bg-gray-300 dark:hover:bg-gray-900"  onClick={() => router.push(`/constituencyposts(id)/${id}`)}>
