        ...
        if (id || userpost) {
          ...
          if (imageUrls && imageUrls.length > 0) {
            ...
            const deleteImagePromises = imageUrls.map((url, index) => {
              const imageRef = ref(storage, `constituency/${id}/image-${index}`);
          ...
          if (post?.data()?.video) {
            await deleteObject(ref(storage, `constituency/${id}/video`));
          }
        } catch (error) {
             ...
             <Link href={`/userProfile/${uid}`} className="h-12 w-12 rounded-md mr-1">
               ...
               />
                 ...
                 <div className="flex-1">
                   ...
                   {post?.data()?.citeInput ? (
                     <div>
                   <p onClick={() => router.push(`/constituencyposts(id)/${id}`)} className="text-md">{post?.data()?.citeInput}</p>
                 ...
                 <Badge className="py-0 max-w-20 truncate " color="gray" icon={HiClock}>
                   ...
                   <p className="line-clamp-3 w-96 break-words  text-sm sm:text-[16px]" onClick={() => router.push(`/constituencyposts(id)/${id}`)}>{post?.data()?.text}</p>
