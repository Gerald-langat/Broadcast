...
export default function CommentModal() {
  ...
  async function sendComment() {
    if(loading) return;
    setLoading(true);
    const docRef = await addDoc(collection(db, "ward",  postId, "comments"), {
      comment: input,
      name: userData.name,
     ...
     const imgRef = ref(storage, `ward/${docRef.id}/images`);
               if (selectedFile) {
                  await uploadString(imgRef, selectedFile, "data_url").then(async () => {
                    const downloadURL = await getDownloadURL(imgRef);
                    await updateDoc(doc(db, "ward", postId, "comments", docRef.id), {
                      images: downloadURL,
                    });
                  });
                }
                
                const vidRef = ref(storage, `ward/${docRef.id}/videos`);
               if (selectedVidFile) {
