...
export default function Comment({ comment, commentId, originalPostId }) {
  ...
  async function deleteComment() {
    ...
    if (window.confirm("Are you sure you want to delete this post?")) {
      deleteDoc(doc(db, "ward", userData.ward, originalPostId, "comments", commentId));
    }
  ...
  return (
    <div className="flex p-3 cursor-pointer border-b border-gray-300 dark:border-gray-600 pl-20">
      {/* user image */}
      <img
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center justify-between">
          ...
          {/* dot icon */}
          <DotsHorizontalIcon className="h-10  w-10 dark:text-gray-100 text-gray-600 hover:bg-sky-100 hover:text-sky-500 p-2 rounded-full cursor-pointer dark:hover:bg-neutral-700" />
        </div>
        ...
        <div className="flex justify-between text-gray-500 dark:text-gray-300 p-2">
          ...
          <Tooltip content='unlike' arrow={false} placement="bottom" className="p-1 text-xs bg-gray-500 -mt-1">
              ...
              <HeartIconFilled
                onClick={likeComment}
                className="h-9 w-9 p-2 text-red-700 hover:bg-blue-100 dark:hover:bg-neutral-700 rounded-full cursor-pointer"
              />
            </Tooltip>
            ) : (
          <Tooltip content='like' arrow={false} placement="bottom" className="p-1 text-xs bg-gray-500 -mt-1">
              <HeartIcon
                onClick={likeComment}
                className="h-9 w-9 p-2 hover:text-red-600 hover:bg-blue-100 dark:hover:bg-neutral-700 rounded-full cursor-pointer"
              />
          </Tooltip>
            )}
            {likes.length > 0 && (
          ...
          <Tooltip content='delete' arrow={false} placement="bottom" className="p-1 text-xs bg-gray-500 -mt-1">
            <TrashIcon
              ...
              className="h-9 w-9 p-2 hover:text-red-600 hover:bg-red-100 rounded-full cursor-pointer dark:hover:bg-neutral-700"
            />
          </Tooltip>
          )}
          ...
          <ReplyIcon className="h-9 w-9 p-2 hover:text-sky-500 hover:bg-sky-100 cursor-pointer rounded-full dark:hover:bg-neutral-700" />
        </Tooltip>
        </div>
      </div>
    </div>
