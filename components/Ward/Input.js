      ...
      if (userData) {
        ...
        if (selectedFiles.length > 0) {
          selectedFiles.forEach((file, index) => {
            const imageRef = ref(storage, `ward/${docRef.id}/image-${index}`);
          ...
          await updateDoc(doc(db, "ward", collectionName, docRef.id), {
            images: imageUrls,
          });
        }
        
        const vidRef = ref(storage, `ward/${docRef.id}/wardVideo`);
      ...
      {userData && (
        <div className="flex border-[1px] rounded-md bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-900 p-3 space-x-3 z-10 top-0 mt-1">
          ...
          <div className="w-full divide-y divide-gray-200 dark:divide-gray-900">
            <div className="">
              <textarea
                className="dark:bg-gray-950 dark:placeholder:text-gray-300 dark:text-gray-300 w-full border-none focus:ring-0 text-lg placeholder-gray-700 tracking-wide min-h-[50px] text-gray-700"
                rows="2"
...
{loading ? (
        ...
        <>
          ...
          <Tooltip content='image' arrow={false} placement="bottom" className="p-1 text-xs bg-gray-500 -mt-1">
                      ...
                      <PhotographIcon className="h-10 w-10 dark:hover:bg-neutral-700 p-2 text-sky-500 hover:bg-sky-100 rounded-full cursor-pointer" />
                      <input
                        type="file"
                        hidden
                        accept="image/*"
          ...
          <Tooltip content='video' arrow={false} placement="bottom" className="p-1 text-xs bg-gray-500 -mt-1">
                      ...
                      <CameraIcon className="h-10 w-10 dark:hover:bg-neutral-700 p-2 text-sky-500 hover:bg-sky-100 rounded-full cursor-pointer" />
                      <input
                        type="file"
                        accept="video/*"
                        hidden
