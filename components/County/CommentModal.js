...
export default function CommentModal() {
 ...
 const { user } = useUser();
  ...
  async function sendComment() {
    ...
    const docRef = await addDoc(collection(db, "county",  postId, "comments"), {
      comment: input,
     ...
     const imgRef = ref(storage, `county/${docRef.id}/images`);
           if (selectedFile) {
              await uploadString(imgRef, selectedFile, "data_url").then(async () => {
                const downloadURL = await getDownloadURL(imgRef);
                await updateDoc(doc(db, "county", postId, "comments", docRef.id), {
                  images: downloadURL,
                });
              });
            }
            
            const vidRef = ref(storage, `county/${docRef.id}/videos`);
           if (selectedVidFile) {
