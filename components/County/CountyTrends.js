...
import { ChartBarIcon, ChatIcon, DotsHorizontalIcon, HeartIcon, PencilAltIcon, ReplyIcon, ShareIcon, ThumbUpIcon, TrashIcon } from '@heroicons/react/outline';
...
const deleteRepost = async () => {
  ...
  return (
    <div className='pt-1 px-2 border-[1px] dark:border-gray-900 rounded-md border-gray-300 sm:min-w-full mt-1'>
    {loading ? (
      ...
      {post?.data()?.citeInput ? (<div><p onClick={() => router.push(`/countyposts(id)/${id}`)}></p>{post?.data()?.citeInput}
        <div className="border rounded-md dark:border-gray-700 border-gray-300 hover:bg-gray-300 dark:hover:bg-neutral-700 cursor-pointer"  onClick={() => router.push(`/countyposts(id)/${id}`)}>
